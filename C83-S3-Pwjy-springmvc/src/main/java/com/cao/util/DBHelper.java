...
public class DBHelper {
	...
	public Connection openConnection() {
		String url = "jdbc:mysql://localhost:3306/damai?useOldAliasMetadataBehavior=true"; // 数据库的地址
	 ...
	 * 	执行查询语句
	 * @param sql
	 * @param params 传数组就可以直接调用，多参数对象 Object...
	 * @return
	 */
		...
		try {
			...
			while (rs.next()) {
				...
				for (int i = 0; i < rsmd.getColumnCount(); i++) {
					...
					/**
					 * 判断当前的列值的类型, 如果是 oracle 的使用TIMESTAMP, mysql使用Timestamp
